...
void WayLandEventLoop::WayLandWaitEventsTimeout(double timeout){
    ...
    while (display_->IsValid()) {
        ...
        //FLWAY_ERROR << "DEBUG:EventLoop Loop "<<std::endl;
        ...
        if (count >= 1) {
            ...
            if(wake_event & POLLIN){
                //FLWAY_ERROR << "DEBUG:EventLoop Wakeup "<<std::endl;
            ...
            if (event & POLLIN) {
                //FLWAY_ERROR << "DEBUG:Wayland Dispatch Once "<<std::endl;
...
void WayLandEventLoop::WayLandWakeUp(){
    ...
    FLWAY_ERROR << "Post an Wakeup Event"<< std::endl;
    if(-1 == ret)
    {
